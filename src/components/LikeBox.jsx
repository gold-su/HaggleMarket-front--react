...
export default function LikeBox({
  ...
  const resolveImg = (url) => (!url ? "/no-image.png" : (url.startsWith("http") ? url : `${BASE}${url}`));
        ...
        <div className="likebox-body">
          ...
          {totalPages > 1 && (
            <div className="likebox-pager">
              ...
              <span className="pager-indicator">{page + 1}/{totalPages}</span>
              <button className="pager-btn" onClick={next} disabled={page === totalPages - 1} aria-label="다음">
                ›
              </button>
            </div>
          )}
