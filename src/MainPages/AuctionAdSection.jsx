...
function AuctionAdSection() {
  ...
  const centerCard = (index = 0) => {
    ...
    const delta =
      cardRect.left - rootRect.left - (root.clientWidth - card.clientWidth) / 2;
    // 현재 스크롤 기준 보정
    root.scrollBy({ left: delta, behavior: "smooth" });
  };
  ...
  return (
    <section className="auction-ad-section">
      ...
      {error && (
        <div style={{ color: "#ef4444" }}>목록을 불러오지 못했습니다.</div>
      )}
      <div className="auction-items-wrapper">
        {" "}
        {/* 스크롤 버튼과 아이템 컨테이너를 감싸는 래퍼 */}
        <button
        ...
        <div className="auction-items-preview" ref={auctionContainerRef}>
          ...
          {auctionItems.map(
            (
              item // slice(0,3) 제거하여 모든 아이템 렌더링
            ) => (
              <div
              ...
              >
                ...
                <div className="auction-item-details">
                  ...
                  <div className="auction-item-description">
                    {item.description}
                  </div>
                  {/* 가격/통계 푸터 */}
