...
function ProductCard({ product, mode = 'used', link, endsAt }) { //product -> 상품 정보 객체 (제목, 가격, 이미지, 내용 등) / mode = 'used' 또는 'auction' / link -> 클릭 시 이동할 경로
  ...
  const imageSrc = useMemo(() => {  //imageSrc -> 상품 이미지 URL 생성
  ...
  const targetLink = link //link prop이 있으면 우선
    ? link //없으면 mode에 따라 분기
    : (mode === 'auction' ? `/auction/detail/${product.id}` : `/products/detail/${product.id}`); // 경매 → /auction/{id} / 중고 → /products/detail/{id}

  const handleFavoriteClick = (e) => { //이벤트 핸들러: 찜하기 클릭
