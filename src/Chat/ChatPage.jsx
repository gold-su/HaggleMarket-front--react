  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        else if (botRoom) setSelectedChatRoomId(botRoom.roomId);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 🔹 최신 메시지가 항상 아래로 오도록 시간순 정렬
        const sorted = [...data].sort(
          (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
        );
        const nextSeen = new Set(sorted.map((m) => m.id || m.clientMsgId));
