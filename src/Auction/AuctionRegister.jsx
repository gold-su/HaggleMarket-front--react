...
function AuctionRegister() {
  ...
  const categoryText = useMemo(() => {
    const parts = [selectedLargeCategory, selectedMiddleCategory, selectedSmallCategory].filter(Boolean);
    return parts.join(' > ');
  ...
  const userNo = 1; // 데모: 고정. (JWT에서 userNo 파싱 or 서버에서 추출하는 방식이면 파라미터 제거)
  ...
  const handleSubmitAuction = async (e) => {
    ...
    try {
      ...
      navigate(`/auction/detail/${auctionId}`); // 상세 페이지로 이동 (라우트에 맞게 수정)
    } catch (e) {
      console.error(e);
        ...
        <section className={stylesLayout.section}>
          ...
          <ul className={stylesLayout.formGroups}>
            ...
            <li className={stylesLayout.formGroup}>
              <div className={stylesLayout.formLabel}>상품 이미지 <small>({imageFiles.length}/{maxImages})</small></div>
              <div className={stylesLayout.formContent}>
                ...
                <div className={stylesForm.formHint}>최대 {maxImages}장, PNG/JPG만 가능. 썸네일을 클릭하면 1번으로 이동합니다.</div>
              </div>
            </li>
                  ...
                  <ul className={stylesForm.categoryList}>
                    {Object.keys(categoriesData).map((cat) => (
                      <li key={cat} className={`${stylesForm.categoryItem} ${selectedLargeCategory === cat ? stylesForm.active : ''}`}>
                        <button type="button" onClick={() => handleLargeCategoryClick(cat)}>{cat}</button>
                      </li>
                    ))}
                  </ul>
                </div>
                    ...
                    {selectedLargeCategory
                      ? Object.keys(categoriesData[selectedLargeCategory]).map((cat) => (
                        <li key={cat} className={`${stylesForm.categoryItem} ${selectedMiddleCategory === cat ? stylesForm.active : ''}`}>
                          <button type="button" onClick={() => handleMiddleCategoryClick(cat)}>{cat}</button>
                        </li>
                      ))
                    ...
                    {selectedLargeCategory && selectedMiddleCategory
                      ? categoriesData[selectedLargeCategory][selectedMiddleCategory].map((cat) => (
                        <li key={cat} className={`${stylesForm.categoryItem} ${selectedSmallCategory === cat ? stylesForm.active : ''}`}>
                          <button type="button" onClick={() => handleSmallCategoryClick(cat)}>{cat}</button>
                        </li>
                      ))
